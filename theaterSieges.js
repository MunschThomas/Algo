...
*/
const columnRowSieges = [];

const theaterSieges = () => {
  
  let columnRowSiegesArray = []; 
    for (let i = 1; i <= 26; i++) {
    columnRowSiegesArray = []
    for (let j = 1; j <= 100; j++) { 
    columnRowSiegesArray.push(`${i}-${j}`)
    }
    columnRowSieges.push(columnRowSiegesArray);
   }
  return columnRowSieges
}
